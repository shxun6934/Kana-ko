...
public class MizuameActivity  extends Activity implements View.OnTouchListener {
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (v.getId()) {
            case R.id.mizuame_kome_imageview:       //コメの場合
                x = (int) event.getRawX();
                y = (int) event.getRawY();
                if (event.getAction() == MotionEvent.ACTION_MOVE) {
                    ...
                    oldX = x;
                    oldY = y;
                } else if (event.getAction() == MotionEvent.ACTION_DOWN) {  //ドラッグされている時
                    oldX = x;
                    oldY = y;
                } else if (event.getAction() == MotionEvent.ACTION_UP) {    //ドラッグし終わった時
                ...
                break;
            case R.id.mizuame_mugi_imageview:       //ムギの場合
                x = (int) event.getRawX();
                y = (int) event.getRawY();
                if (event.getAction() == MotionEvent.ACTION_MOVE) {
